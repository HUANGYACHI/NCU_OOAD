...
public class CanvasPanelHandler extends PanelHandler
{
		...
		for (int i = 0; i < members.size(); i ++)
		{
					...
					&& isSelect == false)
			{
				switch (core.isFuncComponent(members.elementAt(i)))
				{
					case 0:
						// modify 2
						BasicClass curClass = ((BasicClass) members.elementAt(i));
						curClass.setSelect(true);
						selectComp.add(members.elementAt(i));
						isSelect = true;
						// modify 2
						highlightClassLine(e.getPoint(), curClass, members.elementAt(i));
						break;
					case 1:
						...
						isSelect = true;
						// modify 2
						highlightUseCaseLine(e.getPoint(), curUsecase, members.elementAt(i));
						break;
					case 5:
						Point p = e.getPoint();
								...
								p))
						{
							...
							for (int j = 0; j < groupContainer.getComponentCount(); j++) {
								...
								{
									switch (((Core) core).isFuncComponent(groupContainer.getComponent(j)))
									{
										case 0:
											highlightClassLine(p, ((BasicClass) groupContainer.getComponent(j)), (JPanel) groupContainer.getComponent(j));
											break;
										case 1:
											highlightUseCaseLine(p, ((UseCase) groupContainer.getComponent(i)), (JPanel) groupContainer.getComponent(j));
											break;
										default:
											break;
									}
	...
	// modify 2
	void highlightClassLine(Point e, BasicClass curObj, JPanel curJpanel)
	{
		// 上
		if (e.x > curObj.getPort(3).x &&
				e.x < curObj.getPort(3).x + curObj.getSelectBoxSize() * 2 &&
				e.y > curObj.getPort(3).y &&
				e.y < curObj.getPort(3).y + curObj.getSelectBoxSize())
		{
			System.out.println("3上");
			setLineHighlight(3, curJpanel);
		}
		// 下
		else if (e.x > curObj.getPort(0).x &&
				e.x < curObj.getPort(0).x + curObj.getSelectBoxSize() * 2 &&
				e.y > curObj.getPort(0).y &&
				e.y < curObj.getPort(0).y + curObj.getSelectBoxSize())
		{
			System.out.println("0下");
			setLineHighlight(0, curJpanel);
		}
		// 左
		else if (e.x > curObj.getPort(1).x &&
				e.x < curObj.getPort(1).x + curObj.getSelectBoxSize() &&
				e.y > curObj.getPort(1).y &&
				e.y < curObj.getPort(1).y + curObj.getSelectBoxSize() * 2)
		{
			System.out.println("1左");
			setLineHighlight(1, curJpanel);
		}
		// 右
		else if (e.x > curObj.getPort(2).x &&
				e.x < curObj.getPort(2).x + curObj.getSelectBoxSize() &&
				e.y > curObj.getPort(2).y &&
				e.y < curObj.getPort(2).y + curObj.getSelectBoxSize() * 2)
		{
			System.out.println("2右");
			setLineHighlight(2, curJpanel);
		}
	}

	void highlightUseCaseLine(Point e, UseCase curObj, JPanel curJpanel)
	{
		// 上
		if (e.x > curObj.getPort(3).x &&
				e.x < curObj.getPort(3).x + curObj.getSelectBoxSize() * 2 &&
	...
	void setLineHighlight(int side, JPanel curJpanel)
	{
		for (int j = 0; j < lines.size(); j ++)
		{
			...
			{
				case 2:
					if ((curJpanel == ((AssociationLine) lines.elementAt(j)).getFromObj() &&
						((AssociationLine) lines.elementAt(j)).getFromSide() == side) ||
						(curJpanel == ((AssociationLine) lines.elementAt(j)).getToObj() &&
					...
					break;
				case 3:
					if ((curJpanel == ((CompositionLine) lines.elementAt(j)).getFromObj() &&
						((CompositionLine) lines.elementAt(j)).getFromSide() == side) ||
						(curJpanel == ((CompositionLine) lines.elementAt(j)).getToObj() &&
					...
					break;
				case 4:
					if ((curJpanel == ((GeneralizationLine) lines.elementAt(j)).getFromObj() &&
						((GeneralizationLine) lines.elementAt(j)).getFromSide() == side) ||
						(curJpanel == ((GeneralizationLine) lines.elementAt(j)).getToObj() &&
					...
					break;
				case 6:
					if ((curJpanel == ((DependencyLine) lines.elementAt(j)).getFromObj() &&
						((DependencyLine) lines.elementAt(j)).getFromSide() == side) ||
						(curJpanel == ((DependencyLine) lines.elementAt(j)).getToObj() &&
